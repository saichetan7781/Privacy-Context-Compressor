...
struct CompressionPolicy: Equatable {
    ...
    var recommendedMode: String {
        if qualityWeight > 0.38 { return "Conservative Core ML cache compression" }
        if latencyWeight > 0.30 { return "Low-latency on-device compression" }
        if memoryWeight > 0.20 { return "Memory-pressure aware compression" }
        return "Balanced adaptive compression"
    }
}
